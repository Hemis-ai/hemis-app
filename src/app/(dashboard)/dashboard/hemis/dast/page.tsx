...
import type { Severity, DastScan, DastFinding, DastScanProgress, DastFindingStatus } from '@/lib/types'
...
export default function DastPage() {
  ...
  const [targetValidation, setTargetValidation] = useState<{ reachable?: boolean; error?: string; checking?: boolean } | null>(null)
  ...
  // ── Validate target URL ──
  async function validateTarget(url: string) {
    if (!url.trim()) return
    setTargetValidation({ checking: true })
    try {
      const res = await fetch('/api/dast/validate-target', {
        method: 'POST',
      ...
      if (res.ok) {
        const data = await res.json()
        setTargetValidation({ reachable: data.reachable, error: data.error })
      } else {
  ...
  async function cancelScan(scanId: string) {
    try {
      const res = await fetch(`/api/dast/scans/${scanId}/cancel`, { method: 'POST' })
      if (res.ok) {
        setIsScanning(false)
  ...
  async function deleteScan(scanId: string) {
    try {
      const res = await fetch(`/api/dast/scans/${scanId}`, { method: 'DELETE' })
      if (res.ok) fetchScans()
    } catch { /* ignore */ }
  }

  // ── Update finding status ──
  async function updateFindingStatus(findingId: string, status: DastFindingStatus) {
    try {
      const res = await fetch(`/api/dast/findings/${findingId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (res.ok) {
        const data = await res.json()
        setFindings(prev => prev.map(f => f.id === findingId ? { ...f, status: data.finding?.status ?? status } : f))
        if (selectedFinding?.id === findingId) setSelectedFinding(prev => prev ? { ...prev, status } : prev)
      }
    } catch { /* ignore */ }
  }

  // ── Generate report ──
  async function generateReport(scanId: string, format: 'pdf' | 'json' | 'csv') {
    setIsGeneratingReport(true)
    try {
      const res = await fetch(`/api/dast/reports/${scanId}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      <div style={{ flex: 1, padding: '24px 28px', overflowY: 'auto' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
          <div>
            ...
            <div className="mono" style={{ fontSize: 11, color: 'var(--color-text-dim)', letterSpacing: '0.08em', marginTop: 2, paddingLeft: 30 }}>
          ...
          <div style={{ display: 'flex', gap: 8, alignItems: 'flex-start' }}>
            ...
            {selectedScan.status === 'COMPLETED' && (
              <div style={{ display: 'inline-block' }}>
                <button
                  onClick={() => generateReport(selectedScan.id, reportFormat)}
                ...
                >
                  {isGeneratingReport ? 'GENERATING...' : `EXPORT ${reportFormat.toUpperCase()}`}
                </button>
                <div style={{ display: 'flex', gap: 4, marginTop: 4 }}>
                  {(['pdf', 'json', 'csv'] as const).map(fmt => (
                    <button
                      ...
                      className="mono"
                      style={{
                        background: reportFormat === fmt ? 'var(--color-dast-dim)' : 'transparent',
                        border: `1px solid ${reportFormat === fmt ? 'var(--color-dast)' : 'var(--color-border)'}`,
                        color: reportFormat === fmt ? 'var(--color-dast)' : 'var(--color-text-dim)',
                        padding: '2px 8px',
                        fontSize: 9,
                        cursor: 'pointer',
                        letterSpacing: '0.1em',
        ...
        {showNewScan && (
          <div className="bracket-card bracket-dast" style={{ padding: 20, marginBottom: 20 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 14 }}>
              <div>
                ...
                <input
                  ...
                  style={{ marginTop: 4 }}
                  value={newScanUrl}
                  onChange={e => { setNewScanUrl(e.target.value); setTargetValidation(null) }}
                  onBlur={() => validateTarget(newScanUrl)}
                />
                {targetValidation && (
                  <div className="mono" style={{ fontSize: 10, marginTop: 4, color: targetValidation.checking ? 'var(--color-text-dim)' : targetValidation.reachable ? 'var(--color-scanner)' : 'var(--color-critical)' }}>
                    {targetValidation.checking ? 'Checking...' : targetValidation.reachable ? 'Target reachable' : `Unreachable: ${targetValidation.error || 'Connection failed'}`}
                  </div>
                )}
              </div>
              <div>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              <button
                onClick={() => { setShowNewScan(false); setTargetValidation(null) }}
                style={{
                  background: 'transparent',
          ...
          <div className="bracket-card bracket-dast" style={{ padding: 20, marginBottom: 20, position: 'relative', overflow: 'hidden' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                ...
                <button
                  onClick={() => cancelScan(scanProgress.scanId)}
                  style={{
                    background: 'transparent',
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  ...
                  {(s.status === 'COMPLETED' || s.status === 'FAILED') && (
                    <button
                      ...
                      style={{ background: 'none', border: 'none', color: 'var(--color-text-dim)', cursor: 'pointer', fontSize: 12, padding: '0 2px' }}
                      title="Delete scan"
                    >
                      ✕
                    </button>
        ...
        {selectedScan.executiveSummary && (
          <div className="bracket-card bracket-dast" style={{ padding: '16px 18px', marginBottom: 20 }}>
            <div className="mono" style={{ fontSize: 10, fontWeight: 600, letterSpacing: '0.14em', color: 'var(--color-dast)', marginBottom: 8 }}>
              EXECUTIVE SUMMARY
            </div>
        ...
        <div style={{ background: 'var(--color-bg-surface)', border: '1px solid var(--color-border)' }}>
          ...
          <div className="mono" style={{
            display: 'grid',
            gridTemplateColumns: '80px 1fr 100px 80px 70px 60px',
            padding: '8px 14px',
          ...
          {filteredFindings.map(f => (
            ...
            >
              ...
              <span className="mono" style={{ fontSize: 9, color: statusColor(f.status), letterSpacing: '0.05em' }}>{f.status}</span>
...
function FindingDetail({ finding, onClose, onUpdateStatus }: {
  finding: DastFinding
  onClose: () => void
  onUpdateStatus: (id: string, status: DastFindingStatus) => void
}) {
  ...
  return (
    <div style={{ padding: '20px 18px' }}>
      ...
      <div style={{ display: 'flex', gap: 6, marginBottom: 14, flexWrap: 'wrap' }}>
        {(['OPEN', 'ACKNOWLEDGED', 'IN_PROGRESS', 'REMEDIATED', 'FALSE_POSITIVE'] as const).map(s => (
          <button
            key={s}
            onClick={() => onUpdateStatus(finding.id, s)}
            className="mono"
            style={{
              background: finding.status === s ? 'var(--color-dast-dim)' : 'transparent',
              border: `1px solid ${finding.status === s ? 'var(--color-dast)' : 'var(--color-border)'}`,
              color: finding.status === s ? 'var(--color-dast)' : 'var(--color-text-dim)',
              padding: '3px 8px',
              fontSize: 9,
...
function statusColor(status: string): string {
  switch (status) {
    ...
    case 'IN_PROGRESS': return 'var(--color-medium)'
    case 'ACKNOWLEDGED': return 'var(--color-high)'
    case 'REMEDIATED': return 'var(--color-scanner)'
    case 'FALSE_POSITIVE': return 'var(--color-text-dim)'
